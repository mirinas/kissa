# Azure container registry

resource "azurerm_container_registry" "kissa_acr" {
  name                = "kissacontainer"
  resource_group_name = azurerm_resource_group.kissa.name
  location            = azurerm_resource_group.kissa.location
  sku                 = "Basic"
  admin_enabled       = true
}

resource "azurerm_container_registry_task" "kissa_task_api" {
  name                = "purgetask_api"
  container_registry_id = azurerm_container_registry.kissa_acr.id

  platform {
    os = "Linux"
  }

  encoded_step {
    task_content = <<EOF
    version: v1.1.0
    steps:
      - cmd: az acr login --name kissacontainer
      - cmd: acr purge --registry kissacontainer --filter 'REPO:kissa-api-dev.*' --untagged --ago 1h
    EOF
    context_path = "/dev/null"
  }

  timer_trigger {
    name     = "t1"
    schedule = "0 * * * *" // Deletes every hour
    enabled  = true
  }
}

resource "azurerm_container_registry_task" "kissa_task__api_dev" {
  name                = "purgetask_api_dev"
  container_registry_id = azurerm_container_registry.kissa_acr.id

  platform {
    os = "Linux"
  }

  encoded_step {
    task_content = <<EOF
    version: v1.1.0
    steps:
      - cmd: az acr login --name kissacontainer
      - cmd: acr purge --registry kissacontainer --filter 'REPO:kissa-api-dev.*' --untagged --ago 1h
    EOF
    context_path = "/dev/null"
  }

  timer_trigger {
    name     = "t1"
    schedule = "0 * * * *" // Deletes every hour
    enabled  = true
  }
}

resource "azurerm_container_registry_task" "kissa_task_web" {
  name                = "purgetask_web"
  container_registry_id = azurerm_container_registry.kissa_acr.id

  platform {
    os = "Linux"
  }

  encoded_step {
    task_content = <<EOF
    version: v1.1.0
    steps:
      - cmd: az acr login --name kissacontainer
      - cmd: acr purge --registry kissacontainer --filter 'REPO:kissa-api-dev.*' --untagged --ago 1h
    EOF
    context_path = "/dev/null"
  }

  timer_trigger {
    name     = "t1"
    schedule = "0 * * * *" // Deletes every hour
    enabled  = true
  }
}